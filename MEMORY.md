...
- WF05 was partially rebuilt so a Code node prepares Telegram requests and a separate HTTP Request node sends them. A 2026-05-14 live delivery attempt reached the Telegram HTTP node but the n8n server timed out connecting to `api.telegram.org`; package delivery through Telegram remains the next infrastructure issue to solve, while rendered mp4 URLs are valid.
