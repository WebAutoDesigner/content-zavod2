...
- WF05 was rebuilt so a Code node prepares a full Telegram delivery payload, sends it to the local server relay `http://127.0.0.1:8005/send-package`, and the relay reads local `/root/renders/...` mp4 files before sending them through the Cloudflare Worker `dark-field-8e65.nikitagpt2114.workers.dev` to Telegram. This keeps content-factory delivery separate from site domains such as `cvit-auto.ru`. Live WF05 has the Telegram bot token and target chat id filled in n8n; tracked JSON backups must keep placeholders. On 2026-05-14 a safe Worker `getMe` test from n8n returned `ok: true` for bot `KontentZavBot`; the temporary test workflow was deleted afterward.
...
- Later on 2026-05-14 more WF05 delivery diagnostics showed: direct multipart upload from the VPS to `api.telegram.org/sendVideo` times out even with IPv4, while Cloudflare Worker can reach Telegram successfully. The production fix is now: WF05 posts JSON to `/root/telegram-relay-server` on port `8005`; the relay reads local render files or downloads a fallback URL, builds multipart form data, posts it to the Cloudflare Worker, and the Worker forwards it to Telegram. The relay keeps the old `/bot...` JSON proxy behavior and adds `/send-package` for content-factory mp4 delivery. Worker health and relay health were verified; full package resend was intentionally not run after the fix to avoid duplicate Telegram videos.
